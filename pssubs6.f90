      ...
      subroutine newpsdoc(y_top,ibox,time)
	  use mycommons
      integer thispage,totpage
      integer ibox
      real*8 y_top,xr,xl,sc,sch,scv,xshift,xmid
      ...
      sc=sc*1.3
!      xshift=350.-1.1*(xright-xleft)*sc
!      yshift=175.-ybot
      xshift=100.-xleft*sc
      yshift=100.-ybot*sc
      write(psunit,10) '%!PS'
...
!      write(psunit,10) ' x1 y1 rad s1 th cross'
...
!      write(psunit,10) '2.1 2.1 scale'
!      write(psunit,10) '90 rotate'
!      write(psunit,10) '0 -600 translate'
      !write(psunit,10) '2 setlinewidth'
      write(psunit,45) nint(x1), nint(y1-.5*xd),' moveto'
      write(psunit,10) '/Helvetica findfont 5 scalefont setfont'
      write(psunit,50) '(T = ',time,') show'
	  write(psunit,50) '(    Step = ',dble(step),') show'
 ...
 50   format(a,f10.0,a)
	...
	subroutine endpspage()
	use mycommons
	
	write(psunit,10) 'showpage'
	write(psunit,10) 'grestore'
