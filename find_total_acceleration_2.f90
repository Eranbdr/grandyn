PROGRAM find_accel
	...
	grain_vel = 0
	step_changes = 0
	open(unit=changes_file,file='changes__NN_10.csv')
	...
	do step=1, steps
		write(changes_file, '(I6)' , advance='no') step
		do amp=1, numps
			do wl=1, sizes(amp)
				do grain=1, grains
					...
					grain_vel_sq(wl,amp,grain) = grain_vel_sq(wl,amp,grain) + act_accel(wl,amp,grain)*act_accel(wl,amp,grain)
	...
	enddo
	close(changes_file)
	open(totals, file='totals__NN_10.csv')
	open(averages, file='averages__NN_10.csv')
